...
//        photoButton.frame=CGRect(x: photoButton.frame.minX-50, y: photoButton.frame.minY, width: photoButton.frame.width+100, height: photoButton.frame.height)
//        print(photoButton.frame);
        ...
        qrCodeFrameView?.frame = CGRect(x: 25, y: view.frame.height/4, width: view.frame.width-50, height: view.frame.width-50)
        
        barFrameView = UIView()
        barFrameView?.layer.borderColor = UIColor.green.cgColor
        barFrameView?.layer.borderWidth = 3
        barFrameView?.frame = CGRect(x: (qrCodeFrameView?.frame.minX)!, y: (qrCodeFrameView?.frame.minY)!, width: (qrCodeFrameView?.frame.width)!, height: (qrCodeFrameView?.frame.height)!/15)
...
//        view.bringSubview(toFront: barFrameView!)
//        view.bringSubview(toFront: webView!)
        
        resView = resultViewController()
    ...
    func transitionToCropView(image:UIImage?)->Void{
        if(image == nil){
            return
        }
        
        if((image?.size.width)!<CGFloat(100)){
            return
        }
	...
	// MARK: upload Photos
    func upload(image:UIImage?)->Void{
        ...
        let url = NSURL(string: "http://ldzy.cs.columbia.edu:3000/api/photo")
...
//        body.append("hi\r\n".data(using: String.Encoding.utf8)!)
        ...
        let task = session.dataTask(with: request as URLRequest) {
            ...
            guard let _:NSData = data as NSData?, let res:URLResponse = response, error == nil else {
                print("error")
                return
            }
            ...
            print(res)
            print("?")
            self.resView?.dataString = dataString as String?
            flag0=true
...
//        var flag1=false
        ...
        UIView.transition(from: self.view, to: self.resView!.view, duration: 0.1, options: UIViewAnimationOptions.curveEaseIn, completion: nil)
		...
		sessionQueue.async {
			...
			let photoCaptureDelegate = PhotoCaptureDelegate(with: photoSettings, willCapturePhotoAnimation: {
					...
					*/
                    ...
                    let f = LastPhotoRetriever()
                    f.queryLastPhoto(resizeTo: nil, queryCallback: self.transitionToCropView)
...
//        print("out async")
    ...
    func changeViewToWait() {
        let graylayer = UIView(frame: self.view.frame);
        graylayer.backgroundColor = UIColor(colorLiteralRed: 0, green: 0, blue: 0, alpha: 0.75);
        self.view.addSubview(graylayer);
//        photoButton.setTitle("Decoding", for: UIControlState.normal)
        photoButton.isHidden=true

        let progressHUD = ProgressHUD(text: "Processing")
        self.view.addSubview(progressHUD)
//        photoButton.titleLabel?.text = "Decoding";
//        let actindicator = UIActivityIndicatorView(activityIndicatorStyle: UIActivityIndicatorViewStyle.whiteLarge);
